...
public class GhostTrack : Ghost
{
    ...
    private void ChangeWayPoint()
    {
        ...
        else if (m_pacstudent.m_pacstudentState == PacStudent.Pacstudent_Invicible)//吃豆人无敌状态下，远离吃豆人
        {
            ...
            {
                if (Vector2.Distance(wayPoint.position, m_pacstudent.transform.position) > distance)
                {
                    distance = Vector2.Distance(wayPoint.position, m_pacstudent.transform.position);
                    nextWay = wayPoint;
                }
            }
            m_wayPoint = nextWay;
